...
class RansomwareBuilder:
    def __init__(self, root):
        self.root = root
        self.root.title("🔐 ARES-7 Ransomware Builder v4.8")
    ...
    def build_frame(self):
        ...
        version_label = ttk.Label(header, text="v4.8", 
    ...
    def create_bottom_panel(self):
        ...
        # Строка статуса (основная)
        self.status_var = tk.StringVar(value="✅ Готов к сборке")
        self.status_label = ttk.Label(status_frame, textvariable=self.status_var, 
                                     font=("Segoe UI", 10), foreground=self.colors["gray"])
        self.status_label.pack(side="left")
        
        # Дополнительная строка для деталей (НОВАЯ!)
        self.detail_var = tk.StringVar(value="")
        self.detail_label = ttk.Label(status_frame, textvariable=self.detail_var, 
    ...
    def _update_status(self, main_msg, detail_msg="", color="#ffaa00"):
        """Обновляет статус (вызывается из любого потока)"""
    ...
    def _build(self):
        """Основная логика сборки с ДЕТАЛЬНЫМ СТАТУСОМ"""
        ...
        self._update_status("📋 Параметры собраны", 
            f"Диски: {', '.join(selected_drives)} | Расширений: {len(selected_exts)}")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self._update_status(f"✅ PyInstaller найден", f"Версия: {version if version else 'встроенный'}")
            self._update_status("🔧 Сборка EXE через PyInstaller...", "Этап 1/3: Анализ кода")
            ...
            while True:
                ...
                if output:
                    ...
                    output_clean = output.strip()[:60]
                    
                    if "building" in output_lower:
                        self._update_status("🔧 Сборка EXE (этап 2/3: сборка)...", output_clean)
                    elif "analyzing" in output_lower:
                        self._update_status("🔧 Сборка EXE (этап 1/3: анализ)...", output_clean)
                    elif "collecting" in output_lower:
                        self._update_status("🔧 Сборка EXE (этап 3/3: упаковка)...", output_clean)
                    elif "warn" in output_lower:
                        self._update_status(f"⚠️ Предупреждение", output_clean)
                    elif "error" in output_lower:
                        self._update_status(f"❌ Ошибка PyInstaller", output_clean)
                    else:
                        # Обновляем детали даже если нет ключевых слов
                        if output_clean and len(output_clean) > 5:
                            self.detail_var.set(output_clean)
            ...
            if os.path.exists(final_path):
                self._update_status(f"✅ Успешно! Файл: {final_path}", 
                    f"Размер: {self._get_file_size(final_path)}", 
    ...
    def _show_error(self, title, message):
        ...
        self._update_status(f"❌ {message[:50]}...", "", color=self.colors["error"])
